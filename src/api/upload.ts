...
class UploadApi {
  ...
  async uploadFiles(type: UploadType, files: File[]): Promise<UploadResponse> {
    ...
    const { data } = await http.post<UploadResponse>(`/upload/${type}`, formData, {
      timeout: 300000,
    })
    return data
  }
  ...
  async batchImport(type: UploadType, data: any[], version?: number): Promise<BatchImportResponse> {
    ...
    const { data: body } = await http.post<BatchImportResponse>(
      `/upload/batch/${type}`,
      requestBody,
      { timeout: 120000 },
    )
  ...
  // 获取上传统计（与其它 */stats 接口一致：仅 http.get，不单独传 headers，Authorization 只由 @/utils/http 拦截器统一附加）
